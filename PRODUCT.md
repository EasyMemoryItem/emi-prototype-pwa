...
- `CACHE_VERSION` in `sw.js` is stamped from the commit SHA by the Pages deploy workflow, so a change to `index.html`, an icon, or `vendor/` invalidates installed caches on its own. New precached files still go in `PRECACHE` by hand.
